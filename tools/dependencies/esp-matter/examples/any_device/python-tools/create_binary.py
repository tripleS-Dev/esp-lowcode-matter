import json
from matter_enums import AttributeFlags, CommandFlags, EndpointFlags, ClusterFlags, calculate_flag_value, print_flag_dictionary
...
def get_esp_matter_attribute_type(attribute, attribute_type):
    ...
    additional_mapping = {
        "ESP_MATTER_VAL_TYPE_UINT8": [
            "action_id", "fabric_idx", "status", "percent"
        ],
        "ESP_MATTER_VAL_TYPE_UINT16": [
            "endpoint_no", "group_id", "vendor_id", "percent100ths"
        ],
        "ESP_MATTER_VAL_TYPE_UINT32": [
            "cluster_id", "attrib_id", "field_id", "event_id", "command_id",
            "trans_id", "devtype_id", "data_ver", "epoch_s", "elapsed_s"
        ],
        "ESP_MATTER_VAL_TYPE_INT64": [
            "amperage_ma", "energy_mwh", "power_mw", "voltage_mv"
        ],
        "ESP_MATTER_VAL_TYPE_UINT64": [
            "event_no", "fabric_id", "node_id", "bitmap64", "epoch_us",
            "posix_ms", "systime_ms", "systime_us"
        ],
...
def process_endpoint(endpoint):
    # print("Creating endpoint {}".format(endpoint['number']))
    ...
    # print("proto_msg", proto_msg.SerializeToString().hex())
...
def process_device_type(device_type, endpoint_id):
    # print("Adding device type {} version {}".format(device_type['code'], device_type['version']))
    proto_msg = emdm_pb2.FunctionCall()
...
def process_cluster(cluster, endpoint_id):
    ...
    # print("Creating cluster {} revision {} flag {}".format(cluster['code'], cluster['revision'], computed_cluster_flags))
    proto_msg = emdm_pb2.FunctionCall()
...
def set_bounds_value(bounds_val, esp_matter_attribute_type, value):
    if esp_matter_attribute_type in ['ESP_MATTER_VAL_TYPE_BOOLEAN', 'ESP_MATTER_VAL_TYPE_FLOAT',\
     'ESP_MATTER_VAL_TYPE_CHAR_STRING', 'ESP_MATTER_VAL_TYPE_LONG_CHAR_STRING', \
     'ESP_MATTER_VAL_TYPE_OCTET_STRING', 'ESP_MATTER_VAL_TYPE_LONG_OCTET_STRING',\
     'ESP_MATTER_VAL_TYPE_ARRAY']:
        return
    elif esp_matter_attribute_type == 'ESP_MATTER_VAL_TYPE_INT8':
        bounds_val.i8 = value
    elif esp_matter_attribute_type in ['ESP_MATTER_VAL_TYPE_UINT8', 'ESP_MATTER_VAL_TYPE_ENUM8']:
        bounds_val.u8 = value
    elif esp_matter_attribute_type == 'ESP_MATTER_VAL_TYPE_INT16':
        bounds_val.i16 = value
    elif esp_matter_attribute_type in ['ESP_MATTER_VAL_TYPE_UINT16', 'ESP_MATTER_VAL_TYPE_ENUM16']:
    ...
    elif esp_matter_attribute_type in ['ESP_MATTER_VAL_TYPE_BITMAP8', 'ESP_MATTER_VAL_TYPE_BITMAP16', 'ESP_MATTER_VAL_TYPE_BITMAP32']:
        bounds_val.u32 = value
...
def process_attribute(attribute, endpoint_id, cluster_id):
    ...
    # print("Creating attribute {} - flags {} type {} and value {}".format(attribute['definition']['name'], computed_attribute_flags, esp_matter_attribute_type, esp_matter_value))

    proto_msg = emdm_pb2.FunctionCall()
    proto_msg.function = emdm_pb2.FunctionCall.FunctionType.CREATE_ATTRIBUTE
    proto_msg.create_attribute_params.endpoint_id = endpoint_id  # Example endpoint, adjust as needed
    proto_msg.create_attribute_params.cluster_id = cluster_id   # Example cluster, adjust as needed
    ...
    if esp_matter_value is not None:
        ...
        elif esp_matter_attribute_type in ['ESP_MATTER_VAL_TYPE_CHAR_STRING', 'ESP_MATTER_VAL_TYPE_LONG_CHAR_STRING']:
            attr_val.val.char_string = esp_matter_value
        elif esp_matter_attribute_type in ['ESP_MATTER_VAL_TYPE_OCTET_STRING', 'ESP_MATTER_VAL_TYPE_LONG_OCTET_STRING']:
    ...
    if attribute['definition'].get('bounds') is not None:
        ...
        # Set the bounds_min
        set_bounds_value(proto_msg.create_attribute_params.bounds_min, esp_matter_attribute_type, min_value)
...
def process_command(command, endpoint_id, cluster_id):
    ...
    # print("Creating command! cluster_id {} - code {} flags {}".format(cluster_id, command['code'], computed_command_flags))
    proto_msg = emdm_pb2.FunctionCall()
    ...
    if command.get('generated') is not None:
        ...
        # print("Creating generated command! cluster_id {} - code {} flags {}".format(cluster_id, generated_command['code'], computed_generated_flags))
        proto_msg = emdm_pb2.FunctionCall()
        proto_msg.function = emdm_pb2.FunctionCall.FunctionType.CREATE_COMMAND
        proto_msg.create_command_params.endpoint_id = endpoint_id
        proto_msg.create_command_params.cluster_id = cluster_id
...
def process_event(event, endpoint_id, cluster_id):
    # print("Creating event {}".format(event['code']))
    proto_msg = emdm_pb2.FunctionCall()
